                            ...
                            )
{
    ...
    unsigned long src_pixIdx   =  src_batch_index[id_z] + (id_x  + id_y * max_src_width[id_z] ) * plnpkdindex;//((id_x + start_x[id_z])  + (id_y + start_y[id_z]) * max_src_width[id_z] ) * plnpkdindex ;
    if(local_flip == 1) {src_pixIdx =  src_batch_index[id_z] + ((src_width[id_z] -1 -(id_x + start_x[id_z])) +  (id_y + start_y[id_z]) * max_src_width[id_z]) * plnpkdindex; }
    ...
    if((id_x < dst_width[id_z]) && (id_y < dst_height[id_z]))
    {
        ...
        {
            output[dst_pixIdx] = (unsigned char)saturate_8u(255.0 * ((input[src_pixIdx] - local_mean) / local_std_dev)); 
